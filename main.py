...
class SuzukiReactionPredictor:
            ...
            for i in [1, 2]:
                if f'subs{i}_smiles' in df.columns:
                    df[f'subs{i}_mol'] = df[f'subs{i}_smiles'].apply(
                        lambda x: Chem.MolFromSmiles(str(x)) if pd.notna(x) and str(x) not in ['nan', 'None'] else None
                    )
                    df[f'subs{i}_ha'] = df[f'subs{i}_mol'].apply(
                        lambda x: x.GetNumHeavyAtoms() if x else np.nan
                    )
                    df[f'subs{i}_rb'] = df[f'subs{i}_mol'].apply(
                        lambda x: Descriptors.NumRotatableBonds(x) if x else np.nan
                    )
                    df[f'subs{i}_logp'] = df[f'subs{i}_mol'].apply(
    ...
    def predict_yield(self, reaction_conditions):
        ...
        try:
            ...
            for i in [1, 2]:
                if f'subs{i}_smiles' in reaction_conditions:
                    ...
                    if smiles and str(smiles) not in ['nan', 'None']:
                        ...
                        input_df[f'subs{i}_ha'] = mol.GetNumHeavyAtoms() if mol else np.nan
            ...
            return max(0, min(100, round(prediction, 2))), None
        except Exception as e:
            traceback.print_exc()
        ...
        try:
            ...
            for catalyst in catalysts[:20]:
                ...
                if yield_pred is not None:
                    results.append((catalyst, yield_pred))

            return sorted(results, key=lambda x: -x[1])[:5], None
        except Exception as e:
            traceback.print_exc()
    ...
    def get_top_catalysts_from_data(self, n=5):
        ...
        return (self.original_df.groupby('catalizor')['yield']
                .agg(['mean', 'count', 'std'])
    ...
    def visualize_molecules(self, smiles_list, legends=None):
        try:
            mols = [Chem.MolFromSmiles(str(smiles)) for smiles in smiles_list 
                   if smiles and str(smiles) not in ['nan', 'None']]
            ...
            img = Draw.MolsToGridImage(
                ...
                molsPerRow=min(2, len(valid_mols)),
                subImgSize=(300, 300)
            )
